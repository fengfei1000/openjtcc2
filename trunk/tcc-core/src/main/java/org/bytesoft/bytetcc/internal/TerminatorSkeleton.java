...
import org.bytesoft.bytetcc.xa.TxException;
...
//		}
	...
	public void commit(Xid xid, boolean opc) throws XAException {
		try {
			...
			TxException xae = new TxException(TxException.XAER_REMOTE);
			xae.initCause(ex);
			throw xae;
		} catch (HeuristicMixedException ex) {
			XAException xae = new XAException(TxException.XA_HEURMIX);
			xae.initCause(ex);
			throw xae;
		} catch (HeuristicRollbackException ex) {
			XAException xae = new XAException(TxException.XA_HEURRB);
			xae.initCause(ex);
			throw xae;
		} catch (SystemException ex) {
			XAException xae = new XAException(TxException.XAER_RMERR);
			xae.initCause(ex);
			throw xae;
	...
	public void rollback(Xid xid) throws XAException {
		try {
			...
			XAException xae = new XAException(TxException.XA_HEURCOM);
			xae.initCause(ex);
			throw xae;
		} catch (SystemException ex) {
			XAException xae = new XAException(TxException.XAER_RMERR);
